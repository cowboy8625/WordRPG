...
class MainWindow( Frame ):
	def __init__(self, screen):
		super(MainWindow, self).__init__(screen,
										...
										has_shadow = False,
										name = "My Form")
		self._init_gui( screen )


	def _init_gui( self, screen ):
		# TODO: Look into setting up a custom palette
		self.set_theme( "bright" )

		self.layout = Layout( [ 1, 18, 1 ] )
		self.add_layout(self.layout)

		self._reset_button = Button( "Reset", self._reset )
		self.layout.add_widget( Label( "Group 1:" ), 1 )
		self.layout.add_widget( TextBox( 5,
								  label="My First Box:",
								  name="TA",
		...
		self.layout.add_widget(DatePicker("Date",
									 name="DATE",
									 year_range=range(1999, 2100),
									 on_change=self._on_change), 1)
		self.layout.add_widget(
			TimePicker("Time", name="TIME", on_change=self._on_change, seconds=True), 1)
		self.layout.add_widget(Text("Password", name="PWD", on_change=self._on_change, hide_char="*"), 1)
		self.layout.add_widget(DropdownList(
			[("Item 1", 1),
			 ("Item 2", 2),
			 ("Item 3", 3),
			 ("Item 3", 4),
